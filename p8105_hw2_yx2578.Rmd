...
* There are `r (filter(p2_df, vending == "NO" & entry == 1) %>% distinct(station_name, line) %>% nrow() / filter(p2_df, vending == "NO") %>% distinct(station_name, line) %>% nrow()) %>% round(digits = 4) * 100`% station entrances/exits without vending allow entrance.
