...
* There are `r nrow(p3_df1)` records in total.
...
* There are `r nrow(p3_df2)` records in total.
...
* There are `r nrow(p3_df3)` records in total.
* The covered year range in this dataset is from `r min(p3_df3[,1])` to `r max(p3_df3[,1])`.

#### Description of the merged dataset

* The three dayasets are merged by year and month. The joined dataset has a dimention of (`r nrow(p3_join_result)` rows * `r ncol(p3_join_result)` columns). 
* The covered year range in the merged data is from `r min(p3_join_result[,1])` to `r max(p3_join_result[,1])`. 
* The columns in the joined dataset are: `r ls(p3_join_result)`.
