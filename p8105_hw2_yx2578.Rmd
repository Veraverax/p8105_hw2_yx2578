...
* There are `r (filter(p2_df, vending == "NO" & entry == 0) %>% distinct(station_name, line) %>% nrow() / filter(p2_df, vending == "NO") %>% distinct(p2_df, station_name, line) %>% nrow()) %>% round(digits = 4) * 100`% station entrances/exits without vending allow entrance.
